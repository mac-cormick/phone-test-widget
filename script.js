define(['jquery'], function($){
	var CustomWidget = function () {
      ...
      $.get('//'+window.location.host+'/private/api/contact_search.php?SEARCH='+79991112233, function(res){
      	notify_data.element = { id: $(res).find('root > contacts > contact > id').eq(0).text(), type: "contact" };
      	notify_data.from = $(res).find('root > contacts > contact > name').eq(0).text();
      	notify_data.company = $(res).find('root > contacts > contact > company > name').eq(0).text();
      	notify_data.to = 'Анатолий Моляков';
